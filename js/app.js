    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                ...
                if (document.querySelector(".menu__item")) document.addEventListener("click", (function(e) {
                    if (e.target.closest(".menu__item")) menuClose();
                }));
            }));
        }
        function menuClose() {
        ...
        if (inputMasks.length) flsModules.inputmask = Inputmask("+99 (999) 999 99 99").mask(inputMasks);
        ...
        function initSliders() {
            if (document.querySelector(".slider-first")) new swiper_core_Swiper(".slider-first", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                loop: true,
                navigation: {
                    prevEl: ".slider-first__button-prev",
                    nextEl: ".slider-first__button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            if (document.querySelector(".slider-second")) new swiper_core_Swiper(".slider-second", {
                modules: [ Navigation ],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
                ...
                loop: true,
                navigation: {
                    prevEl: ".slider-second__button-prev",
                    nextEl: ".slider-second__button-next"
                },
                breakpoints: {
                    320: {
                        slidesPerView: 1,
            ...
            animationFrame() {
                ...
                this.elements.forEach((el => {
                    const parameters = {
                        ...
                        direction: el.dataset.direction ? el.dataset.direction + "0.2" : "-0.2",
                        coefficient: el.dataset.coefficient ? Number(el.dataset.coefficient) : 2,
...
***************************************************************************** */
        ...
        const calc = document.querySelector(".calc__form"), weightInput = document.querySelector(".calc__input-weight"), heightInput = document.querySelector(".calc__input-height"), calcBtn = document.querySelector(".calc__button"), resultDesc = [ {
            num: 18.5,
            grade: "У вас недостатня вага",
            desc: "Для набору нормальної ваги та покращення якості тіла приходьте на тренування до Microfitness. Наші тренери підберуть для вас оптимальну програму тренувань та раціон харчування."
        }, {
            num: 24.9,
            grade: "У вас нормальна вага",
            desc: "Для підтримання нормальної ваги та покращення якості тіла приходьте на тренування до Microfitness. Наші тренери підберуть для вас оптимальну програму тренувань та раціон харчування."
        }, {
            num: 29.9,
            grade: "У вас є зайва вага",
            desc: "Для отримання нормальної ваги та покращення якості тіла приходьте на тренування до Microfitness. Наші тренери підберуть для вас оптимальну програму тренувань та раціон харчування."
        }, {
            num: 30,
            grade: "У вас є підозра на наявність ожиріння",
            desc: "Зверніться будь ласка до лікаря та с заключенням лікаря наші тренери допоможуть підібрати программу тренувань яка не нашкодить вашому здоров'ю."
        } ];
        calcBtn.addEventListener("click", showResult);
        let result = 0;
        function calcIbm() {
            if (weightInput.value !== "" && heightInput.value !== "" && weightInput.value > 0 && heightInput.value > 0) result = (parseFloat(weightInput.value) / (parseFloat(heightInput.value) / 100) ** 2).toFixed(1); else result = weightInput.value === "" || weightInput.value <= 0 ? "Вірно вкажіть вашу вагу" : "Вірно вкажіть ваш зріст";
            return result;
        }
        function resultCompare() {
            ...
            let resultGrade = "";
            let resultText = "";
            for (const item of resultDesc) if (result <= item.num) {
                resultGrade = item.grade;
                resultText = item.desc;
                break;
            } else {
                resultGrade = item.grade;
                resultText = item.desc;
        ...
        function showResult() {
            const {resultGrade, resultText} = resultCompare();
            let calcResult = calc.querySelector(".calc__result");
            if (!calcResult) {
                calcResult = document.createElement("div");
                calcResult.classList.add("calc__result");
                calc.insertAdjacentElement("beforeend", calcResult);
            } else calcResult.innerHTML = "";
            if (isNaN(result)) calcResult.innerHTML = `<p class="accent accent--red">${result}</p>`; else {
                calcResult.innerHTML = `\n            <p>Ваш ІМТ = <span class="accent accent--red">${result}</span></p>\n            <p class="accent accent--red">${resultGrade}</p>\n            <p>${resultText}</p>\n        `;
                weightInput.value = "";
                heightInput.value = "";
            }
        }
